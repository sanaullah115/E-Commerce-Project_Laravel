                    ...
                    <tr>
                        <th scope="row">
                            <div class="d-flex align-items-center">
                                <img src="{{asset('/Upload/Product/'.$cart->product->image)}}"
                                    class="img-fluid rounded-circle" style="width: 80px; height: 80px;" alt="">

                            </div>
                        </th>
                        <td>
                            <p class="mb-0 mt-4">{{ $cart->product->title }}
                            </p>
                        </td>
                        <td>
                            <p class="mb-0 mt-4">${{ $cart->product->price }}
                            </p>
                        </td>
                        <td data-cart_id="{{$cart->id}}" data-price="{{$cart->product->price}}">
                        ...
                        <td>
                            <p class="mb-0 mt-4 total_product_price">${{$cart->product->price * $cart->Quantity}}</p>
                        </td>
                        <td>
                ...
                <div class="bg-light rounded">
                    <div class="p-4">
                        ...
                        <div class="d-flex justify-content-between mb-4">
                            ...
                            <p class="mb-0 total_payout">$0.00</p>
                        </div>
                    ...
                    <div class="py-4 mb-4 border-top border-bottom d-flex justify-content-between">
                        ...
                        <p class="mb-0 pe-4 total_payout">$0.00</p>
                    </div>
...
<script>
    ...
    $(document).on('click','.cart-plus-minus',function() {
        ...
        $.ajax({
            ...
            type: 'post',
            data: { _token: '{{ csrf_token() }}',cart_id: cart_id,quantity: quantity},
    ...
    function total_payout(){
        $.ajax({
            ...
            success: function(response) {
                $('.total_payout').text("$" + response.totalpayout.toFixed(2));
            }
        });
    }
</script>
